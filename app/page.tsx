...
function DashboardView({ usuario, onLogout, onStartQuiz }: { usuario: Usuario; onLogout: () => void; onStartQuiz: (topic: string, questions: QuizQuestion[]) => void }) {
  ...
  const [selectedPuntaje, setSelectedPuntaje] = useState<PuntajeHistorial | null>(null)
  const [preguntasGuardadas, setPreguntasGuardadas] = useState<PreguntaGuardada[]>([])
  ...
  const handleVerPreguntas = async (p: PuntajeHistorial) => {
    if (selectedPuntaje?.id === p.id) { setSelectedPuntaje(null); setPreguntasGuardadas([]); return }
    setSelectedPuntaje(p)
    setLoadingPregs(true)
    try {
      const res = await fetch(`/api/puntajes?puntajeId=${p.id}`)
      const data = await res.json()
      setPreguntasGuardadas(data.preguntas ?? [])
    } catch { setPreguntasGuardadas([]) }
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                {historial.map((p, i) => (
                  <div key={p.id} style={{ animationDelay:`${i * 0.05}s` }}>
                    <div className="hist-card" style={{ cursor:"pointer" }} onClick={() => handleVerPreguntas(p)}>
                      <div>
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                        ...
                        <ChevronRight style={{ width:16, height:16, color:"rgba(255,255,255,.2)", transform: selectedPuntaje?.id === p.id ? "rotate(90deg)" : "none", transition:"transform .2s" }} />
                      </div>
                    </div>
                    {selectedPuntaje?.id === p.id && (
                      <div style={{ marginTop:8, display:"flex", flexDirection:"column", gap:8, paddingLeft:8 }}>
                        {loadingPregs ? (
                          <p style={{ fontSize:12, color:"rgba(255,255,255,.3)", padding:"12px 0" }}>Cargando preguntas...</p>
                        ) : preguntasGuardadas.length === 0 ? (
                          <p style={{ fontSize:12, color:"rgba(255,255,255,.3)", padding:"12px 0" }}>No hay preguntas guardadas para este quiz.</p>
                        ) : (
                          preguntasGuardadas.map((q, qi) => (
                            <div key={q.id} style={{ background:"rgba(255,255,255,.02)", border:"1px solid rgba(255,255,255,.06)", borderRadius:10, padding:"12px 14px", animation:"fadeIn .3s ease forwards" }}>
                              <p style={{ fontSize:13, fontWeight:500, color:"#fff", marginBottom:8 }}>{qi + 1}. {q.enunciado}</p>
                              {q.opciones.map(op => (
                                <div key={op.id} style={{ fontSize:12, padding:"4px 8px", borderRadius:6, marginBottom:3, background: op.esCorrecta ? "rgba(16,185,129,.12)" : "transparent", color: op.esCorrecta ? "#10b981" : "rgba(255,255,255,.35)" }}>
                                  {op.esCorrecta ? "✓ " : "  "}{op.texto}
                                </div>
                              ))}
                            </div>
                          ))
          ...
          <div style={{ textAlign:"center", animation:"fadeUp .4s ease forwards" }}>
            <div style={{ marginBottom:40 }}>
              ...
              <p style={{ color:"rgba(255,255,255,.4)", fontSize:15 }}>
                Escribe un tema y la IA generará 5 preguntas personalizadas
              </p>
            </div>
            ...
            <div style={{ maxWidth:520, margin:"0 auto", display:"flex", flexDirection:"column", gap:14 }}>
              ...
              <button className="qb" style={{ height:52, fontSize:15 }} onClick={handleGenerate} disabled={!topic.trim() || isLoading}>
                {isLoading
                  ? <><Loader2 style={{ width:18, height:18, animation:"spin 1s linear infinite" }} /> Generando examen con IA...</>
                  : <><Sparkles style={{ width:18, height:18 }} /> Generar Examen con IA</>}
              </button>
            ...
            {historial.length > 0 && (
              <div style={{ marginTop:56 }}>
                <p style={{ fontSize:12, color:"rgba(255,255,255,.25)", letterSpacing:".08em", textTransform:"uppercase", marginBottom:16 }}>Últimos exámenes</p>
